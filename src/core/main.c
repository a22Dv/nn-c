...
int main() {
  ...
  tensor_t* tensor_am = tensor_emap(tensor_a, random);
  tensor_t* tensor_bm = tensor_emap(tensor_b, random);
  tensor_destroy(&tensor_a);
  tensor_destroy(&tensor_b);

  tensor_t* tensor_c = tensor_contract(tensor_am, tensor_bm);
  tensor_t* scalar_a = tensor_create(0, NULL);
  scalar_a->data[0] = 50;
  tensor_t* tensor_d = tensor_eadd(tensor_c, scalar_a);
  printf("A:\n");
